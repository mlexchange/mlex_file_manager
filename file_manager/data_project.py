...
class DataProject:
    ...
    def init_from_splash(self, splash_uri, project_id=None, api_key=None):
        ...
        while num_elem == 5000:
            elem = requests.post(
                f"{splash_uri}?page%5Boffset%5D={indx}&page%5Blimit%5D=5000",
    ...
    def browse_data(
        ...
        if data_type == "tiled":
            ...
            if browse_format == "*":
                ...
                uris = LocalDataset.filepaths_from_directory(
                    dir_path, browse_format, recursive=recursive
                )
    ...
    def get_table_dict(self):
        ...
        """
        data_table_dict = [
            {"uri": dataset.uri, "type": dataset.type} for dataset in self.data
        ]
    ...
    def tiled_to_local_project(self, subset=None, pattern=r'[/\\?%*:|"<>]'):
        ...
        for unit_project_id in list_project_id:
            ...
            if local_path.exists() and len(unit_uris) == len(
                list(local_path.glob("*.tif"))
            ):
                unit_data_info = pd.read_parquet(
                    f"{local_path}/data_info.parquet", engine="pyarrow"
                )
            else:
                ...
                if subset is not None:
                    filtered_subset = [
                        num - start_indx
                        for num in subset
                        if start_indx <= num < start_indx + len(unit_uris)
                    ]
                ...
                if local_path.exists() and len(unit_uris) > 0:
                    partial_data_info = pd.read_parquet(
                        f"{local_path}/data_info.parquet", engine="pyarrow"
                    )
                    partial_local_uris = partial_data_info["uri"].tolist()
                    filtered = [
                        (uri, indx)
                        for uri, indx in zip(unit_uris, unit_indx)
                        if uri not in partial_local_uris
                    ]
                    if filtered:
                ...
                unit_data_info["local_uri"] = [
                    f"{local_path}/{uid}.tif" for uid in uid_list
                ]
                ...
                if partial_data_info is not None:
                    unit_data_info = pd.concat(
                        [partial_data_info, unit_data_info], ignore_index=True
                    )
                unit_data_info.to_parquet(
                    f"{local_path}/data_info.parquet", engine="pyarrow"
                )
