...
class TiledDataset(Dataset):
    ...
    def _get_response(tiled_uri, expected_shape, resize, max_tries):
        ...
        while status_code != 200 and trials < max_tries:
            if len(expected_shape) == 3:
                response = (
                    requests.get(f"{tiled_uri},0,::5,::5&format=png")
                    if resize
                    else requests.get(f"{tiled_uri},0,:,:&format=png")
                )
            else:
                response = (
                    requests.get(f"{tiled_uri},::5,::5&format=png")
                    if resize
    ...
    def read_data(self, export="base64", resize=True, log=False, max_tries=5):
        ...
        if export == "pillow":
            ...
            return (
                (image.crop((0, 1, *image.size)), self.uri)
                if resize
                else (image, self.uri)
            )
    ...
    def browse_data(
        ...
        if isinstance(tiled_client, Node):
            ...
            for node in nodes:
                ...
                if browse_format != "**/" and recursive:
                    ...
                    tiled_uris = tiled_uris + [
                        f"{mod_tiled_uri}?slice={i}&expected_shape={expected_shape}&dtype={dtype}"
            ...
            if browse_format != "**/" and recursive:
                ...
                expected_shape = "%2C".join(expected_shape)
                tiled_uris = tiled_uris + [
                    f"{tiled_uri}?slice={i}&expected_shape={expected_shape}&dtype={dtype}"
                    for i in range(num_imgs)
                ]
            else:
